...
class Grid:
        ...
        for _ in range(seq_actions):
            if current_cell.edges[dir] != None and current_cell.edges[dir].weighting != Grid.Node.Edge.Weighting.BARRIER:
                ...
                if self.UseGridGenerator:
                    ...
                    print('Moving to ', current_cell)
                    self.set_current_cell(current_cell.grid_cell_number())
            else:
                return False
        return True
    ...
    def updateBarrier(self, cell, dir):
        if cell and cell.edges[dir] != None:
            print('Updating Cell:', cell.grid_cell_number(), dir, cell.edges[dir].weighting)
            ...
            # if (cell.grid_cell_number() == 209 and dir == Grid.Node.Edge.Type.EAST) or (cell.grid_cell_number() == 210 and dir == Grid.Node.Edge.Type.WEST):
    ...
    def update_barriers(self, lidar_img, current_theta):
        ...
        if self.UseGridGenerator:
            ...
            for angle in [0, 270, 180, 90]:
                ...
                if math.isnan(distance) or math.isinf(distance) or distance > Grid.CELL_SIZE*3 :
    ...
    class Path:
        ...
        def extend(self, new_node, grid):
            ...
            self.h = self.k_h1*Grid.Node.h_distance(new_node, new_path.goal_node) + sum([0.05 for cell in new_path.path if cell in grid.visited_cells]) # We encourage exploration 
            new_path.current_weight+=1
            return new_path
    ...
    def call_rec_unvisited_search(self, start_cell: Node.Edge.Type, goal_cell: Node.Edge.Type, **kwargs):
        ...
        self.rec_unvisited_search(start_cell, goal_cell, stop_depth= kwargs['stop_depth'], paths_found=paths_found, max_frontier_size=kwargs['max_frontier_size'])
        with Search_Lock:
            Running_Search = False
            if not isinstance(Return_Search_Value, Grid.Path):
                # Return_Search_Value = paths_found.pop()
                # print('setting val')
    ...
    def step(self, delta):
        ...
        with Search_Lock:
            if Running_Search:
                ...
                print('Reading val')
                self.path_deq = deque(Return_Search_Value.path[1::])
                Return_Search_Value = []
                print(self.path_deq)
                print('plaksjdnsd q')
